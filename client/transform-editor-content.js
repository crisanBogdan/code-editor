...
export function transformEditorContent(text, parser, colors) {
    ...
    const used = new Set()
    const spanOpenTag = '<span', spanClosedTag = '</span>'
        ...
        while (i < text.length) {
            ...
            if (text[i] === value[0]) {
                if (text.slice(i, i + value.length) === value) {
                    const replacement = `<span style="color: ${colors[type]};">${value}</span>`
                    const newText = text.slice(0, i) + replacement + text.slice(i + value.length)
                    text = newText
            ...
            // if encountered a span go past it
                ...
                if (text.slice(i, i + spanOpenTag.length) === spanOpenTag) {
                    ...
                    while (i < text.length) {
                        if (text[i] === '<' && text.slice(i, i + spanClosedTag.length) === spanClosedTag) {
