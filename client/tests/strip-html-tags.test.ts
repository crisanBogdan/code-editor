...
describe('stripHtmlTags', () => {
    ...
    it('string with one tag and with properties should remove tag', () => {
        assert.strictEqual(
            stripHtmlTags('<span style="color: red;" data-testid=\'test\'>Test</span>'), 
            'Test',
        )
    })
    ...
    it('string with multiple tags should remove all tags', () => {
        assert.strictEqual(
            stripHtmlTags('<span style="color: text;">Test</span> <p class="test">paragraph</p>'), 
            'Test paragraph',
        )
    })
})