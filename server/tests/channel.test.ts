...
describe('AppChannel', () => {
    ...
    it(
        'should fail if a new connection has the same username' +
    ...
    it(
        'should send the message to all other connections when someone writes' +
            ' a message',
        () => {
            const channel = new AppChannel('test', config, logger);
            const send = mock.fn();
    ...
    it(
        'should send a message to all connections when someone changes' +
            ' their username',
        () => {
            const channel = new AppChannel('test', config, logger);
            const send = mock.fn();
    ...
    it('should close a connection if the message length is above limit', () => {
        ...
        emitter.emit(
            'message',
            new Array(config.maxWsMessageLength + 1).fill('x').join('')
        );
